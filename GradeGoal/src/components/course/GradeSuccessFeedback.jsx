...
function GradeSuccessFeedback({
  ...
  const currentGrade = gradeResult.success ? gradeResult.courseGrade : 'Ongoing';
  ...
  const gradePercentage = gradeData.score && gradeData.maxScore
  ...
  const getNextActionSuggestions = () => {
    ...
    const assessmentsWithoutScores = Object.values(grades).flat().filter(grade =>
    ...
    if (assessmentsWithoutScores.length > 0) {
      suggestions.push({
        type: 'add_score',
        title: 'Add Score',
        description: `Add scores to ${assessmentsWithoutScores.length} pending assessment${assessmentsWithoutScores.length > 1 ? 's' : ''}`,
    ...
    if (progress < 100) {
      suggestions.push({
        ...
        description: `You're ${Math.round(progress)}% complete with this course`,
      ...
      <div className="bg-white/95 backdrop-blur-md rounded-lg shadow-xl w-full max-w-2xl overflow-hidden border border-white/20">
        <div className={`${colorScheme.primary} text-white p-6 rounded-t-lg`}>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-2xl font-bold">Grade Saved Successfully!</h2>
                <p className="text-green-100">Your assessment has been recorded</p>
              </div>
            </div>
            <button
              onClick={onClose}
        ...
        <div className="p-6 space-y-6">
          <div className="bg-gray-50 rounded-xl p-4">
            <h3 className="text-lg font-semibold text-gray-800 mb-3">Grade Details</h3>
          ...
          <div className="bg-blue-50 rounded-xl p-4">
            <h3 className="text-lg font-semibold text-gray-800 mb-3">Updated Course Statistics</h3>
            <div className="grid grid-cols-2 gap-4">
              <div>
              ...
              <div>
                ...
                <p className="font-semibold text-gray-800">
                  {typeof currentGrade === 'number' ? currentGrade.toFixed(1) : currentGrade}
                </p>
              </div>
            </div>
          </div>
