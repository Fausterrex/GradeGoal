...
function AddCourse({
  ...
  useEffect(() => {
    const loadExistingCategories = async () => {
      if (editingCourse && editingCourse.id) {
        try {
          const categories = await getAssessmentCategoriesByCourseId(editingCourse.id);
  ...
  const synchronizeCategoriesWithDatabase = async (courseId, newCategories, existingCategories) => {
    try {

      const existingCategoryIds = existingCategories.map(cat => cat.id || cat.categoryId);

      const newCategoryIds = newCategories
        .filter(cat => cat.id && typeof cat.id === 'number')
        .map(cat => cat.id);

      const categorySystemChanged = newCategories.length !== existingCategories.length;
      ...
      if (categorySystemChanged) {
        ...
        const categoriesToDelete = existingCategories.slice(newCategories.length);
        ...
        for (let i = 0; i < newCategories.length; i++) {
          ...
          if (newCategory.name !== existingCategory.categoryName ||
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const courseData = {
        ...
        semester: newCourse.semester || 'FIRST',
      ...
      if (editingCourse) {
        ...
        try {
          ...
          const existingCategories = await getAssessmentCategoriesByCourseId(editingCourse.id);
          await synchronizeCategoriesWithDatabase(editingCourse.id, newCourse.categories, existingCategories);
          ...
          if (updatedCourse) {
            ...
            setNewCourse(prev => ({
              ...prev,
              categorySystem: updatedCourse.categorySystem || (newCourse.categories.length === 2 ? '2-categories' : '3-categories')
            }));
      ...
      if (editingCourse) {
        ...
        const originalCourse = existingCourses.find(c => c.id === editingCourse.id);
      } else {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            ...
            <div>
              ...
              <input
                ...
                placeholder="e.g., 3"
              />
              <p className="text-xs text-gray-500 mt-1">Used for GPA calculations</p>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                <select
                  ...
                  onChange={(e) => setNewCourse({...newCourse, categorySystem: e.target.value})}
                  className="w-full px-4 py-3 pr-10 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#8168C5] focus:border-[#8168C5] transition-all duration-200 appearance-none cursor-pointer"
                >
                  <option value="3-categories">3 Categories (Assignments, Quizzes, Exam)</option>
                  <option value="2-categories">2 Categories (Exam, Laboratory Activity)</option>
                </select>
                <div className="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
                  <svg className="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
          ...
          <div className="mb-8">
            <div className="flex justify-between items-center mb-4">
              ...
              <div className="flex gap-3">
                ...
                >
                  Use {newCourse.categorySystem === '3-categories' ? '3-Category' : '2-Category'} Template
                </button>
