 ...
 * 
 * Manages the display and interaction with course cards in the main dashboard.
 * Handles course creation, editing, deletion, and archiving operations.
 * 
 * @param {Function} onCourseUpdate - Callback to update parent component with course changes
 * @param {Function} onCourseSelect - Callback when a course is selected for detailed view
 * @param {Object} grades - Object containing grades for all courses
 * @param {Array} courses - Array of course objects to display
 */
function CourseManager({ onCourseUpdate, onCourseSelect = () => {}, onBack = () => {}, grades = {}, courses = [] }) {
  ...
  useEffect(() => {
    ...
    console.log('Grades changed in CourseManager:', grades);
  }, [grades]);
  
  // Effect to handle course updates
  useEffect(() => {
    console.log('Courses changed in CourseManager:', courses);
    console.log('Archived courses state:', archivedCourses);
    console.log('Show archived state:', showArchived);
  ...
  useEffect(() => {
    if (courses && courses.length > 0) {
      ...
      const archived = courses.filter(course => course.isArchived === true);
      const active = courses.filter(course => course.isArchived !== true);
  ...
  /**
   * Updates the parent component when courses change
   * @param {Array} updatedCourses - The updated array of courses
   */
  ...
  /**
   * Handles course creation/update from AddCourse component
   * @param {Array} updatedCourses - The updated courses array from AddCourse
   */
  ...
  /**
   * Archives a course (hides it from main view but preserves data)
  ...
  const archiveCourse = async (courseId) => {
    if (window.confirm('Archive this course? It will be hidden from the main view but all data will be preserved. You can restore it later from the archive.')) {
      try {
        ...
        if (courseToArchive) {
          // Mark the course as archived locally for immediate UI feedback
          const archivedCourse = { ...courseToArchive, isArchived: true };
          
          // Add to archived courses
          setArchivedCourses(prev => [...prev, archivedCourse]);
          
          // Also update the local courses state to mark this course as archived
  ...
  /**
   * Handles course selection for navigation to GradeEntry
   * @param {Object} course - The selected course (can be active or archived)
   */
  const handleCourseSelection = (course) => {
    ...
    if (isArchived) {
      ...
      const courseWithArchiveStatus = { ...course, isArchived: true };
  ...
  /**
   * Restores an archived course back to the main view
   * @param {number} courseId - The ID of the course to restore
   */
  const restoreCourse = async (courseId) => {
    try {
      ...
      if (courseToRestore) {
        ...
        // Also update the local courses state to mark this course as not archived
    ...
    <div className="w-full p-6 bg-gradient-to-br from-purple-50 via-indigo-50 to-blue-50 h-full bg-[url('./drawables/coursesBG.png')] bg-cover bg-center relative">
      ...
      <div className="absolute inset-0 bg-white/20 backdrop-blur-[2px] pointer-events-none"></div>
       ...
       <div className="flex items-center gap-3 mb-6 bg-white/90 backdrop-blur-md rounded-2xl p-5 shadow-xl border border-white/50 relative z-10">
         ...
         <div className="ml-auto flex items-center gap-4">
           <span className="bg-gradient-to-r from-[#8168C5] to-[#6B5B9A] text-white px-3 py-2 rounded-xl text-xl shadow-md ">{courses.filter(course => !course.isArchived).length} Course{courses.filter(course => !course.isArchived).length !== 1 ? 's' : ''}</span>
      ...
      {showCourses && (
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6 mb-8 relative z-10">
          {courses.filter(course => !course.isArchived).map((course) => {
          ...
          if (course.categories && course.categories.length > 0 && grades && typeof grades === 'object') {
            ...
            console.log(`Course ${course.id} (${course.name}):`, {
              ...
              categoriesData: course.categories.map(c => ({ id: c.id, name: c.name, weight: c.weight })),
              gradesAvailable: Object.keys(grades).length,
            ...
            // Don't immediately set progress to 0 - check if we have any previous progress data
            if (course.categories && course.categories.length > 0) {
              // Try to calculate progress even without grades data
              // This prevents the "0" progress issue when editing courses
              progress = 0; // Start with 0 but will be calculated below
              courseGrade = 'Ongoing';
              hasGrades = false;
              isOngoing = true;
            } else {
          ...
          // This prevents the "0" progress issue when editing courses
          if (progress === 0 && course.categories && course.categories.length > 0 && (!grades || Object.keys(grades).length === 0)) {
            // When grades are temporarily unavailable after editing, show "Ongoing" instead of "0%"
            isOngoing = true;
            hasGrades = false;
            courseGrade = 'Ongoing';
          }
