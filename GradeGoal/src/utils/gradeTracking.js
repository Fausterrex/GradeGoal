/**
 * Grade Tracking Utilities
 * 
 * This module provides utilities for tracking grade progress over time,
 * generating weekly snapshots, and analyzing grade trends.
 */
...
export const generateWeeklySnapshot = (courses, grades, weekStart, weekEnd) => {
  ...
  courses.forEach(course => {
    ...
    course.categories.forEach(category => {
      ...
      if (validGrades.length > 0) {
        const categoryAverage = GradeService.calculateCourseGrade([{
          ...category,
          grades: validGrades
        }], course.gradingScale, course.maxPoints, course.handleMissing);
        
        categorySnapshot.currentAverage = categoryAverage;
        courseSnapshot.currentGPA += categoryAverage * (category.weight || 0) / 100;
      }
    ...
    if (course.gradingScale === 'percentage' || courseSnapshot.currentGPA > 100) {
    ...
    courseSnapshot.progress = calculateCourseProgress(courseSnapshot.categories);
...
export const generateWeeklySnapshots = (courses, grades, weeks = 12) => {
  ...
  for (let i = weeks - 1; i >= 0; i--) {
    ...
    const snapshot = generateWeeklySnapshot(courses, grades, weekStart, weekEnd);
...
export const analyzeGradeTrends = (snapshots) => {
  ...
  const volatility = Math.sqrt(
    weeklyChanges.reduce((sum, change) => sum + change * change, 0) / weeklyChanges.length
  );

  const trend = change > 0.1 ? 'improving' : change < -0.1 ? 'declining' : 'stable';
  const consistency = volatility < 0.2 ? 'high' : volatility < 0.5 ? 'medium' : 'low';
...
export const predictGradeTrends = (snapshots, weeksAhead = 4) => {
  ...
  for (let i = 1; i <= weeksAhead; i++) {
    const predictedGPA = Math.max(0, Math.min(4.0, currentGPA + (averageChange * i)));
    predictions.push({
...
export const generateGradeInsights = (snapshots) => {
  ...
  if (trendAnalysis.trend === 'improving') {
    ...
    insights.push({
      ...
      message: `Your GPA has decreased by ${Math.abs(trendAnalysis.change)} points. It's time to reassess your study strategy.`,
      action: 'Consider seeking help, reviewing study methods, or meeting with instructors.'
    });
  }
  ...
  if (trendAnalysis.consistency === 'low') {
    insights.push({
      ...
      message: 'Your grades are fluctuating significantly. This might indicate inconsistent study habits.',
    ...
    insights.push({
      ...
      message: 'Your grades are stable and consistent. This shows good study discipline.',
      action: 'Maintain your current approach while looking for opportunities to improve.'
    });
  }
  ...
  if (recentSnapshots.length === 2) {
    const recentChange = recentSnapshots[1].overallGPA - recentSnapshots[0].overallGPA;
    if (recentChange > 0.2) {
      insights.push({
        type: 'positive',
        title: 'Recent Improvement',
        message: `Your GPA increased by ${recentChange.toFixed(2)} points this week!`,
        action: 'Identify what worked well this week and continue those practices.'
      });
    } else if (recentChange < -0.2) {
      insights.push({
        type: 'warning',
        title: 'Recent Decline',
        message: `Your GPA decreased by ${Math.abs(recentChange).toFixed(2)} points this week.`,
        action: 'Review what might have caused this decline and adjust accordingly.'
      });
    }
  }
